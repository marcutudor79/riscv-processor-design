...
have generated the assembly instructions that were previously described. An example of such C code
...
the object file from the C source code:

\begin{framed}
riscv64-linux-gnu-gcc -g -O0 -mcmodel=medlow -mabi=ilp32 -march=rv32im -Wall -c -o se201-prog.o se201-prog.c
\end{framed}

A fair mention is that in order to have a compiled program, one needs to provide a main function as
an entry point. What is shown below is the dummy main function in which the addv function is called:
\begin{lstlisting}[language=C, numbers=left, stepnumber=1, numbersep=5pt]
int main() {
    int a[] = {1, 2, 3, 4, 5};
...
several differences exist. Firstly, in this generated assembly, only the stack is used for passing
arguments from the caller to the callee, while in the provided assembly, the registered were used to
pass the arguments.

Secondly, the check for null pointers, which can be observed in the section .L2, .L4 and .L5 is done
using the bnez instruction and instead of jumping to a common return -1, each check contains its own
jump to the return -1 section. This leads to a larger code size and is also less efficient due to the
fact that the positive case needs an extra jump, which normally, flushes the pipeline.

Thirdly, the loop structure is different, as it can be seen in .L8 and .L7 sections.
